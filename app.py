...
# Creates UpdatePasswordForm to create the form to update password
class UpdatePasswordForm(FlaskForm):
    """Form to update the password"""
    # Sets password to new password field
    password = PasswordField('New Password: ', validators=[DataRequired(), Length(min=12, max=25)])
    # Sets submit button to update
    submit = SubmitField('Update')
    if submit:
        # If submit, commits session to database
        db.session.commit()
...
# Sets route to update html based on the id
@app.route('/update/<int:id>', methods=['GET', 'POST'])
@login_required
def update(id):
    """Function to update a users password"""
    # Calls the updatepasswordform
    form = UpdatePasswordForm()
    # Sets variable for the queried database
    userPass = User.query.get(id)
    # If the request method is post
    if request.method == "POST":
        # Sets the current/new password to the database table
        current_user.password = form.password.data
        # Hashes the new password
        bcrypt.generate_password_hash(password=current_user.password)

        try:
            # Tries to commit the changes to the database
            db.session.commit()
            # Prints the user has updated
            print("User Updated Successfully!")
            # Returns the update html
            return render_template("update.html",
                                # Sets form= updatepaswordform
                                form=form,
                                # userPass is this userPass
                                userPass=userPass,
                                # id is this id
                                id=id), \
            ...
            # If database can't commit, throws error
        ...
        # Else, sends user back to update
        return render_template("update.html",
                               # Sets form= updatepaswordform
                               form=form,
                               # userPass is this userPass
